...
<aside class="w-full md:w-64 flex-shrink-0">
    {{-- Header --}}
    <div class="mb-8">
        <span class="text-[#F53003] text-[10px] font-black uppercase tracking-widest">Inventory</span>
        <h2 class="text-3xl font-black uppercase tracking-tighter mb-8 text-[#1b1b18]">Clothes</h2>
    </div>

    {{-- Main Category Navigation --}}
<nav class="space-y-4 mb-10 text-[13px] font-bold uppercase tracking-tight text-gray-800">
    {{-- In-update ang values para mag-match sa Admin Panel Select Options --}}
    <a href="?category[]=T-Shirts" class="hover:text-[#F53003] transition-colors flex justify-between group">
        T-Shirts & Tops
    </a>

    <a href="?category[]=Jersey" class="hover:text-[#F53003] transition-colors flex justify-between group">
        Jerseys
    </a>

    <a href="?category[]=Socks" class="hover:text-[#F53003] transition-colors flex justify-between group">
        Socks
    </a>

    {{-- Bonus: Idinagdag ang Hoodies dahil kasama ito sa Controller logic mo --}}
    <a href="?category[]=Hoodies" class="hover:text-[#F53003] transition-colors flex justify-between group">
        Hoodies & Jackets
    </a>
</nav>
...
<main class="flex-1" x-data="{
            ...
            <div class="mt-auto">
                @auth
                    <div class="flex flex-col sm:flex-row gap-2">
                        <button @click="selectedProduct = { id: '{{ $product->id }}', name: '{{ addslashes($product->name) }}', price: '{{ $product->price }}', sizes: '{{ $product->sizes }}', brand: '{{ $product->brand }}', stock: {{ $product->quantity }}, image: '{{ asset($product->image) }}' }; quantity = 1; openModal = true;"
    ...
    <div x-show="openModal" class="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm" x-cloak>
        <div @click.away="openModal = false" class="bg-white w-full max-w-md p-10 border-t-8 border-[#F53003] shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            {{-- ADDED THIS WRAPPER TO PREVENT ERRORS --}}
