    ...
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row gap-12 px-6 py-16 min-h-screen bg-white">
        <aside class="w-full md:w-64 flex-shrink-0">
            ...
            <div class="mb-8">
                ...
                <h2 class="text-4xl font-black uppercase tracking-tighter">Shoes</h2>
            </div>

            {{-- Main Category Navigation --}}
<nav class="space-y-4 mb-10 text-[13px] font-bold uppercase tracking-tight text-gray-800">
    {{-- Updated to match Admin Panel and Controller categories --}}
    <a href="?category[]=Basketball+Shoes" class="hover:text-[#F53003] transition-colors flex justify-between group">Basketball Shoes</a>
    <a href="?category[]=Running+Shoes" class="hover:text-[#F53003] transition-colors flex justify-between group">Running Shoes</a>
    <a href="?category[]=Jordan+Brand" class="hover:text-[#F53003] transition-colors flex justify-between group">Jordan</a>
    <a href="?category[]=Nike" class="hover:text-[#F53003] transition-colors flex justify-between group">Nike</a>
    <a href="?category[]=Lifestyle+Sneakers" class="hover:text-[#F53003] transition-colors flex justify-between group">Lifestyle</a>
</nav>
            ...
            <div x-data="{
                ...
                applyExclusive(name, value) {
                    ...
                    // Use standard parameter name (the controller handles the array conversion)
                    url.searchParams.set(name + '[]', value);
                    window.location.href = url.href;
                }
            }">
                {{-- Only show filters if they are passed from the controller --}}
                @isset($filters)
                ...
                {{-- Size Filter (Hardcoded as requested) --}}
                <div class="mb-4">
                    ...
                    <div x-show="activeFilter === 'size'" class="grid grid-cols-4 gap-2 pt-4 pb-6 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                        @php $allSizes = array_merge(['S', 'M', 'L', 'XL'], range(36, 48)); @endphp
